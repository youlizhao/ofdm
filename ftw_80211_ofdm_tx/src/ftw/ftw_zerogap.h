...
#ifndef INCLUDED_FTW_OFDM_INSERT_POSTAMBLE_H
#define INCLUDED_FTW_OFDM_INSERT_POSTAMBLE_H

#include <gr_sync_block.h>
#include <vector>

class ftw_zerogap;
typedef boost::shared_ptr<ftw_zerogap> ftw_zerogap_sptr;
...
{
  friend ftw_zerogap_sptr
  ftw_make_zerogap(int symbol_length, int N_symbols,
			       const std::vector<std::vector<gr_complex> > &zerogap);

protected:
  ftw_zerogap(int symbol_length, int N_symbols,
			  const std::vector<std::vector<gr_complex> > &zerogap);
