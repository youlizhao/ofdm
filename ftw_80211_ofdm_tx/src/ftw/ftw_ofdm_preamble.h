...
#ifndef INCLUDED_ftw_ofdm_OFDM_INSERT_PREAMBLE_H
#define INCLUDED_ftw_ofdm_OFDM_INSERT_PREAMBLE_H

#include <gr_sync_block.h>
#include <vector>

class ftw_ofdm_preamble;
typedef boost::shared_ptr<ftw_ofdm_preamble> ftw_ofdm_preamble_sptr;
...
{
  friend ftw_ofdm_preamble_sptr
  ftw_make_ofdm_preamble(int symbol_length, int N_symbols,
			       const std::vector<std::vector<gr_complex> > &preamble);

protected:
  ftw_ofdm_preamble(int symbol_length, int N_symbols,
			  const std::vector<std::vector<gr_complex> > &preamble);
